...
heading = {0:['Source IP', 'Source Port', 'Destination IP', 'Destination Port', 'Protocol', 'Flow Duration',
           ...
           1:['source_ip', 'source_port', 'destination_ip', 'destination_port', 'protocol', 'flow_duration',
                ...
                'backward_p1', 'backward_p2', 'backward_p3', 'backward_p4', 'backward_p5', 'backward_p6', 'backward_p8']}
...
def flow_statistics(times, flow, time, start_time, frames_lenght, table_forward, table_backward, tmp_flow_table,
    ...
    # Create iat features
    features = feature_creation(times)
...
def iat_statistics(file, timeout, label):
    ...
    table_statistics = pd.DataFrame(table_statistics, columns=heading[0])
    table_statistics['label'] = label
    table_statistics.to_csv(
        path_or_buf='tmp/' + os.path.splitext(os.path.splitext(os.path.split(file)[1])[0])[0] + '.csv', index=False)
